...
class _TraySystem:
    ...
    def __init__(self, name: Optional[str] = None, icon_path: Optional[str] = None, more_options: list = None, extra_menu_items: list = None):
        # 延迟导入 pystray
        import pystray
        ...
        if self.hwnd:
            ...
            logger.info("未检测到控制台窗口，托盘将以无窗口模式运行")
        ...
        # 添加调用方自定义的完整 MenuItem 列表（支持 checked/submenu/radio）
        if extra_menu_items:
...
def enable_min_to_tray(name: Optional[str] = None, icon_path: Optional[str] = None, exit_callback=None, more_options: list = None, extra_menu_items: list = None) -> None:
    """
    启用最小化到托盘功能

    如果检测不到控制台窗口（如 .pyw 运行、右键菜单启动），仍会创建托盘图标，
    只是跳过窗口管理相关功能（显示/隐藏菜单、最小化监控）。
    如果在 Linux 等无 GUI 环境下运行，则会跳过。

    Args:
        name: 托盘图标显示的名称，默认使用程序名称
        icon_path: 图标文件路径，默认动态生成
        exit_callback: 退出回调函数，当用户点击托盘退出菜单时调用
        more_options: 额外菜单项列表，简单形式 [(名称, 回调函数), ...]
        extra_menu_items: 调用方自定义的完整 pystray.MenuItem 列表，
                         支持 checked/submenu/radio，会插入到"重启"菜单之前
    """
    ...
    with _lock:
        ...
        # 检测控制台窗口（仅用于决定是否启用窗口管理功能，不再阻断托盘创建）
        hwnd = _get_console_hwnd()
        if not hwnd:
            logger.info("未检测到控制台窗口，托盘将以无窗口管理模式启动（常见于右键菜单启动场景）")

        _tray_instance = _TraySystem(name, icon_path, more_options, extra_menu_items)
