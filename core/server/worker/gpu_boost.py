...
class GpuBoostManager:
    ...
    同时处理 config_update 命令，让子进程同步主进程的运行时配置变更。
    """
    ...
    def handle_command(self, task):
        """处理 GPU 加速 / 配置更新命令任务。"""
        if task.command == 'config_update':
            self._apply_config_update(task)
            return
    ...
    def _apply_config_update(self, task):
        """应用主进程下发的运行时配置变更到当前子进程的 Config 副本。"""
        updates = getattr(task, 'config_updates', None) or {}
        if not updates:
            return
        applied = []
        for key, val in updates.items():
            if hasattr(Config, key):
                ...
                applied.append(f"{key}: {old!r} -> {val!r}")
        if applied:
            logger.info(f"子进程 Config 已同步更新: {', '.join(applied)}")
