        ...
        try:
            ...
            if (task.type == 'file'
                ...
                try:
                    ...
                    logger.warning(f"对齐过程异常（非致命）: {e}")
            
            logger.debug(f"Pipeline: 对齐步骤完成，继续Token拼接")
            ...
            logger.info(f"任务完成: {task.task_id[:8]}, 时长={result.duration:.2f}s, 耗时={process_time:.3f}s, RTF={rtf:.3f}, text={result.text[:50]}, text_accu={result.text_accu[:50] if result.text_accu else 'None'}")
