...
def _handle_remote_transcribe(app, file_path: str):
    """
    处理来自第二个实例的文件转录请求
    ...
    path = Path(file_path)
    if not path.exists():
        logger.error(f"远程转录请求的文件不存在: {file_path}")
        return
    ...
    def _do_transcribe():
        try:
            ...
            transcriber.set_progress_callback(
                lambda p, t, fn=path.name: app.tray_manager.set_transcribe_progress(fn, p, t)
            )
            success = transcriber.transcribe()
            if success:
                logger.info(f"远程转录成功: {path}")
            else:
                logger.error(f"远程转录失败: {path}")
        except Exception as e:
            logger.error(f"远程转录异常: {path}, 错误: {e}", exc_info=True)
        finally:
    ...
    try:
        async for raw_message in websocket:
            ...
            try:
                ...
                # 处理"transcribe_file"特殊消息（来自第二个实例的文件转录请求）
                if data.get('type') == 'transcribe_file':
                    file_path = data.get('path', '')
                    if file_path:
                        logger.info(f"收到远程转录请求: {file_path}")
                        _handle_remote_transcribe(app, file_path)
                    continue
