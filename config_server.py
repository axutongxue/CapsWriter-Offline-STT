...
def _apply_runtime_overrides():
    """从 runtime_config.json 加载覆盖项到 ServerConfig。"""
    if not _RUNTIME_CONFIG_PATH.exists():
        return
    try:
        import json
        overrides = json.loads(_RUNTIME_CONFIG_PATH.read_text(encoding='utf-8'))
...
def save_runtime_overrides(overrides: dict):
    """
    将运行时覆盖项写入 runtime_config.json。
    供托盘菜单切换 GPU 加速、转录模型时调用。
    """
    import json
    try:
        existing = {}
        if _RUNTIME_CONFIG_PATH.exists():
            existing = json.loads(_RUNTIME_CONFIG_PATH.read_text(encoding='utf-8'))
            if not isinstance(existing, dict):
                existing = {}
        existing.update({k: v for k, v in overrides.items() if k in _RUNTIME_OVERRIDABLE_KEYS})
        _RUNTIME_CONFIG_PATH.write_text(
            json.dumps(existing, ensure_ascii=False, indent=2),
            encoding='utf-8'
        )
        # 同步覆盖到当前进程的 ServerConfig
        for key, val in overrides.items():
            if key in _RUNTIME_OVERRIDABLE_KEYS and hasattr(ServerConfig, key):
                setattr(ServerConfig, key, val)
        return True
